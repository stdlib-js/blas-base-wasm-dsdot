...
tape( 'the `main` method has an arity of 5', function test( t ) {
	t.strictEqual( dsdot.main.length, 5, 'returns expected value' );
	t.end();
});
...
tape( 'the `main` method computes the dot product of `x` and `y` with extended accumulation and result', function test( t ) {
	...
	dot = dsdot.main( x.length, x, 1, y, 1 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the `main` method returns `0.0`', function test( t ) {
	...
	dot = dsdot.main( 0, x, 1, y, 1 );
	t.strictEqual( dot, 0.0, 'returns expected value' );

	dot = dsdot.main( -4, x, 1, y, 1 );
	t.strictEqual( dot, 0.0, 'returns expected value' );
	t.end();
});
